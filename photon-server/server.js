...
function extractContactInfo(chatId) {
  if (!chatId) return { phone: null, email: null };
...
async function forwardMessageToBackend(messageData) {
  ...
  try {
    if (!messageData) {
      console.error('[ERROR] forwardMessageToBackend: messageData is null/undefined');
      return;
    }
    
    console.log(`[DEBUG] forwardMessageToBackend: Processing message ${messageId} from chatId: ${messageData.chatId || 'unknown'}`);
    const transformed = transformMessage(messageData);
    if (!transformed) {
      console.error(`[ERROR] forwardMessageToBackend: transformMessage returned null for message ${messageId}`);
      return;
    }
    ...
    if (activeUserId) {
      ...
      console.log(`[DEBUG] forwardMessageToBackend: Forwarding message ${messageId} to backend for userId: ${activeUserId}`);
    } else {
      console.warn(`[WARN] forwardMessageToBackend: No activeUserId set, forwarding message ${messageId} without userId`);
    }
    ...
    console.log(`[DEBUG] forwardMessageToBackend: ✅ Successfully forwarded message ${messageId} to backend`);
  } catch (error) {
    console.error(`[ERROR] forwardMessageToBackend: ❌ Failed to forward message ${messageId}:`, error.message);
    if (error.response) {
      console.error(`[ERROR] forwardMessageToBackend: Backend response status: ${error.response.status}, data:`, error.response.data);
    }
  }
}
  ...
  try {
    ...
    await sdk.startWatching({
      onMessage: async (message) => {
        ...
        console.log(`[DEBUG] startWatching: New message received: ${messageId} from ${message?.sender || 'unknown'} in chat ${message?.chatId || 'unknown'}`);
        forwardMessageToBackend(message).catch(err => {
          console.error(`[ERROR] startWatching: Error forwarding message ${messageId}:`, err.message);
        });
      },
      onError: (error) => {
    ...
    isWatching = true;
    console.log('[DEBUG] startWatching: ✅ Successfully started watching for messages');
    return true;
  } catch (error) {
    console.error('[ERROR] startWatching: ❌ Failed to start watching:', error.message);
    console.error('[ERROR] startWatching: Error stack:', error.stack);
    return false;
  }
}
...
app.get('/health', (req, res) => {
  res.json({ status: isWatching ? 'ready' : 'disconnected', watching: isWatching });
});
...
app.get('/api/chats', async (req, res) => {
  ...
  console.log(`[DEBUG] [${requestId}] Query params:`, JSON.stringify(req.query));
  try {
    const limit = parseInt(req.query.limit) || 100;
    console.log(`[DEBUG] [${requestId}] Calling sdk.listChats({ limit: ${limit}, sortBy: 'recent' })`);
    const chats = await sdk.listChats({ limit, sortBy: 'recent' });
    console.log(`[DEBUG] [${requestId}] ✅ SDK returned ${chats.length} chats (took ${Date.now() - startTime}ms)`);
    
    // Map chats and look up contact names for DMs without displayName
    // OPTIMIZATION: Use cache to avoid redundant AppleScript calls
    const mappedChats = await Promise.all(chats.map(async (chat) => {
      let displayName = chat.displayName;
      
      // If no displayName and it's a DM, try to look up from Contacts app
      if (!displayName && !chat.isGroup) {
        ...
        if (cachedName) {
          ...
          console.log(`[DEBUG] [${requestId}] Using cached contact name for ${chat.chatId}: ${displayName}`);
        } else if (phone || email) {
          ...
          try {
            ...
            if (contactName) {
              ...
              console.log(`[DEBUG] [${requestId}] Found contact name for ${chat.chatId}: ${displayName}`);
            } else {
              console.log(`[DEBUG] [${requestId}] No contact name found for ${chat.chatId} (phone: ${phone || 'N/A'}, email: ${email || 'N/A'})`);
            }
          } catch (error) {
            console.error(`[ERROR] [${requestId}] Contact lookup failed for ${chat.chatId}:`, error.message);
      ...
      return {
        ...
        lastMessageDate: chat.lastMessageAt ? chat.lastMessageAt.toISOString() : null,
        unreadCount: chat.unreadCount
      };
    }));
    
    const totalTime = Date.now() - startTime;
    console.log(`[DEBUG] [${requestId}] ✅ Successfully mapped ${mappedChats.length} chats (total time: ${totalTime}ms)`);
    res.json({ chats: mappedChats });
  } catch (error) {
    const totalTime = Date.now() - startTime;
    console.error(`[ERROR] [${requestId}] ❌ GET /api/chats failed after ${totalTime}ms:`, error.message);
...
app.get('/api/messages', async (req, res) => {
  ...
  try {
    ...
    if (!chatGuid) {
      console.error(`[ERROR] [${requestId}] ❌ Missing required parameter: chatGuid`);
      return res.status(400).json({ error: 'chatGuid is required' });
    }
    
    console.log(`[DEBUG] [${requestId}] Looking up messages for chatGuid: "${chatGuid}" (limit: ${limit}, offset: ${offset})`);
    
    // OPTIMIZATION: Cache listChats results to avoid redundant calls
    // This endpoint is called frequently, so caching helps significantly
    ...
    if (cached && (Date.now() - cached.timestamp) < CACHE_TTL_MS) {
      chats = cached.data;
      console.log(`[DEBUG] [${requestId}] Using cached listChats (${chats.length} chats)`);
    } else {
      console.log(`[DEBUG] [${requestId}] Searching for chat in listChats (searching ${1000} chats)...`);
      chats = await sdk.listChats({ limit: 1000 });
      global[CACHE_KEY] = { data: chats, timestamp: Date.now() };
      console.log(`[DEBUG] [${requestId}] Retrieved ${chats.length} chats from SDK and cached`);
    }
    ...
    if (!matchingChat) {
      console.log(`[DEBUG] [${requestId}] ⚠️ Chat not found in listChats for chatGuid: "${chatGuid}", trying direct query with multiple formats`);
      ...
      let result = { messages: [], total: 0 };
      // SDK doesn't support offset, so we fetch more messages and slice them
      const offsetNum = parseInt(offset) || 0;
      const limitNum = parseInt(limit) || 100;
      for (let i = 0; i < formatsToTry.length; i++) {
        const format = formatsToTry[i];
        console.log(`[DEBUG] [${requestId}] Attempt ${i + 1}/${formatsToTry.length}: Trying chatId format: "${format}"`);
        result = await sdk.getMessages({
          chatId: format,
          limit: offsetNum + limitNum, // Fetch enough to cover offset + limit
          excludeOwnMessages: false
        });
        console.log(`[DEBUG] [${requestId}] Format "${format}" returned ${result.messages.length} messages`);
        if (result.messages.length > 0) {
          console.log(`[DEBUG] [${requestId}] ✅ SUCCESS: Found ${result.messages.length} messages with format: "${format}"`);
          break;
        }
      }
      console.log(`[DEBUG] [${requestId}] Direct query final result: ${result.messages.length} messages (total available: ${result.total})`);
      ...
      const totalTime = Date.now() - startTime;
      console.log(`[DEBUG] [${requestId}] ✅ Returning ${mappedMessages.length} mapped messages (total available: ${result.total}, time: ${totalTime}ms)`);
      return res.json({ messages: mappedMessages, total: result.total });
    }
    
    console.log(`[DEBUG] [${requestId}] ✅ Found matching chat in listChats: chatId="${matchingChat.chatId}", isGroup=${matchingChat.isGroup}, displayName="${matchingChat.displayName || 'null'}"`);
    
    // Use the SDK's chatId from listChats - it should match what getMessages expects
    // For groups, chatId is the GUID. For DMs, it's the service-prefixed identifier.
    let chatIdToUse = matchingChat.chatId;
    
    // For group chats, extract just the GUID part if chatGuid has the full format
    if (matchingChat.isGroup && chatGuid.includes('chat')) {
      ...
      if (guidMatch) {
        ...
        console.log(`[DEBUG] [${requestId}] Extracted group GUID from "${chatGuid}" → "${chatIdToUse}"`);
      }
    }
    
    // Try with the SDK's chatId first
    // SDK doesn't support offset, so we fetch more messages and slice them
    const offsetNum = parseInt(offset) || 0;
    const limitNum = parseInt(limit) || 100;
    console.log(`[DEBUG] [${requestId}] Calling sdk.getMessages({ chatId: "${chatIdToUse}", limit: ${offsetNum + limitNum}, excludeOwnMessages: false })`);
    ...
    console.log(`[DEBUG] [${requestId}] First attempt with chatId "${chatIdToUse}" returned ${result.messages.length} messages (total available: ${result.total})`);
    ...
    if (result.messages.length === 0 && matchingChat.isGroup) {
      ...
      if (guidMatch && guidMatch[0] !== chatIdToUse) {
        ...
        console.log(`[DEBUG] [${requestId}] Retry attempt: Trying group GUID format: "${guidOnly}"`);
        const retryResult = await sdk.getMessages({
          chatId: guidOnly,
          limit: offsetNum + limitNum, // Fetch enough to cover offset + limit
          excludeOwnMessages: false
        });
        console.log(`[DEBUG] [${requestId}] Retry with "${guidOnly}" returned ${retryResult.messages.length} messages`);
        if (retryResult.messages.length > 0) {
          console.log(`[DEBUG] [${requestId}] ✅ SUCCESS: Found ${retryResult.messages.length} messages with format: "${guidOnly}"`);
          result = retryResult;
        }
      }
    }
    ...
    if (result.messages.length === 0 && !matchingChat.isGroup && chatIdToUse.includes(';')) {
      const addressOnly = chatIdToUse.split(';').slice(1).join(';');
      console.log(`[DEBUG] [${requestId}] Retry attempt: Trying DM format without service prefix: "${addressOnly}"`);
      const retryResult = await sdk.getMessages({
        chatId: addressOnly,
        limit: offsetNum + limitNum, // Fetch enough to cover offset + limit
        excludeOwnMessages: false
      });
      console.log(`[DEBUG] [${requestId}] Retry with "${addressOnly}" returned ${retryResult.messages.length} messages`);
      if (retryResult.messages.length > 0) {
        console.log(`[DEBUG] [${requestId}] ✅ SUCCESS: Found ${retryResult.messages.length} messages with format: "${addressOnly}"`);
        result = retryResult;
      }
    }
    ...
    console.log(`[DEBUG] [${requestId}] After pagination: ${messages.length} messages (from ${result.messages.length} fetched, offset: ${offsetNum}, limit: ${limitNum})`);
    ...
    const totalTime = Date.now() - startTime;
    console.log(`[DEBUG] [${requestId}] ✅ Successfully returning ${mappedMessages.length} mapped messages (total available: ${result.total}, time: ${totalTime}ms)`);
    res.json({ messages: mappedMessages, total: result.total });
  } catch (error) {
    const totalTime = Date.now() - startTime;
    console.error(`[ERROR] [${requestId}] ❌ GET /api/messages failed after ${totalTime}ms:`, error.message);
    console.error(`[ERROR] [${requestId}] Stack trace:`, error.stack);
    res.status(500).json({ error: error.message });
  }
});
...
app.post('/api/messages/send', async (req, res) => {
  try {
    ...
    const result = await sdk.send(chatGuid, contentToSend);
    
    // Transform result to include ISO date
...
app.post('/api/connect', async (req, res) => {
  ...
  try {
    ...
    if (started) {
      const totalTime = Date.now() - startTime;
      console.log(`[DEBUG] [${requestId}] ✅ POST /api/connect: Success - watching started (time: ${totalTime}ms)`);
      res.json({ success: true, message: 'Started watching for messages' });
    } else {
      const totalTime = Date.now() - startTime;
      console.error(`[ERROR] [${requestId}] ❌ POST /api/connect: Failed to start watching (time: ${totalTime}ms)`);
      res.status(503).json({ success: false, error: 'Failed to start watching' });
    }
  } catch (error) {
    const totalTime = Date.now() - startTime;
    console.error(`[ERROR] [${requestId}] ❌ POST /api/connect failed after ${totalTime}ms:`, error.message);
    console.error(`[ERROR] [${requestId}] Stack trace:`, error.stack);
    res.status(500).json({ error: error.message });
  }
});
