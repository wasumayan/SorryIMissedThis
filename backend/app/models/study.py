...
class StudyCondition:
    ...
    start_date: datetime
    end_date: datetime
    day_in_condition: int  # 1 (each condition is 1 day)
...
class StudyParticipant:
    ...
    study_end_date: datetime  # study_start_date + 3 days
    ...
    def current_condition_start(self) -> datetime:
        ...
        return self.study_start_date + timedelta(days=self.current_condition_index * 1)
    ...
    def days_in_current_condition(self) -> int:
        """Calculate which day participant is in current condition (always 1 since each condition is 1 day)"""
        # Since each condition is exactly 1 day, we always return 1
        return 1
...
class StudyMetrics:
    ...
    user_id: str
    condition: str
    day: int  # 1 (each condition is 1 day)
    date: datetime
